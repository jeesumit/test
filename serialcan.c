	...
	token = strtok(NULL,",");
     ...
     if(lat !=0.0 && lng !=0.0){
	   ...
	   cte_ab = cross_track_error(dis_ac,ang_ab, ang_ac);
           ...
           //estset = 2*prdstpt;
           cancte = pidCTE(cte_ab,prev_cte,10.1,0.0000001,0.0);
           valcte = (float)cancte + cte_ab;
           prdstpt =ConvertRadtoDeg(asin(valcte / dis_ac));
           estset = prdstpt;
	   ...
	   bytes_written = write(fds,message, sizeof(message));
           ...
           sprintf(logdata,"CTE:%.2f, estCTE:%.2f,pidCTE:%.2f estang:%.2f canBuscte:%d num:%d canbus:%d\n",cte_ab,prev_cte,valcte,estset,cancte,line_count,canbus);
           printf("%s ",logdata); 
